...
import { Loader2, LayoutList, CheckCircle2, Clock, XCircle, FileEdit } from 'lucide-react';
import { getMyPlansAsNutritionist } from '../../services/api';
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((plan) => (
              <div key={plan.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-shrink-0">
                  ...
                  {plan.status === 'Rejected' && plan.rejectionReason && (
                    <p className="text-xs text-red-500 mt-1 max-w-xs text-right">{plan.rejectionReason}</p>
                  )}
                </div>
              </div>
            ))}
