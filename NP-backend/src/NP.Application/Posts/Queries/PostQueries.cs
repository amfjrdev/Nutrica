...
internal sealed class GetPendingPostsQueryHandler : IQueryHandler<GetPendingPostsQuery, List<PostDto>>
{
    private readonly IPostRepository _repo;
    public GetPendingPostsQueryHandler(IPostRepository repo) => _repo = repo;

    public async Task<Result<List<PostDto>>> HandleAsync(GetPendingPostsQuery query, CancellationToken cancellationToken = default)
    {
...
internal sealed class GetPostsByAuthorQueryHandler : IQueryHandler<GetPostsByAuthorQuery, List<PostDto>>
{
    private readonly IPostRepository _repo;
    public GetPostsByAuthorQueryHandler(IPostRepository repo) => _repo = repo;

    public async Task<Result<List<PostDto>>> HandleAsync(GetPostsByAuthorQuery query, CancellationToken cancellationToken = default)
    {
        var list = await _repo.GetByAuthorIdAsync(query.AuthorId, cancellationToken);
        return Result.Success(list.Select(PostMapper.ToDto).ToList());
    }
}
